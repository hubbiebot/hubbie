...
describe('HTTP safety defaults', () => {
  it('returns Problem Details without a stack for unexpected async errors', async () => {
    const response = await request(buildApp(async () => {
    ...
    expect(response.text).not.toContain('connection string must not be exposed');
  ...
  it('rejects a JSON request larger than 256 KB', async () => {
    ...
    expect(response.headers['x-request-id']).toBeTruthy();
    expect(response.body).toMatchObject({ status: 413, title: 'Payload Too Large' });
  });

  it('returns Problem Details with a request ID for an unknown route', async () => {
    const response = await request(buildApp()).get('/missing?token=must-not-appear');
